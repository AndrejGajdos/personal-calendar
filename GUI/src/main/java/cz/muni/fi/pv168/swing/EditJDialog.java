...
import cz.muni.fi.pv168.workers.EditEventWorker;
...
TableEventModel tableEv = new TableEventModel();
...
private void jButton1clickEditEvent(java.awt.event.ActionEvent evt) {    //GEN-FIRST:event_jButton1clickEditEvent
        ...
        if (validateTime()) {
                ...
                EditEventWorker worker = new EditEventWorker(event);
                worker.execute();
...
}    //GEN-LAST:event_jButton1clickEditEvent

//----------------------CANCEL_CLICK---------------------------

private void jButton2clickCancelEdit(java.awt.event.ActionEvent evt) {    //GEN-FIRST:event_jButton2clickCancelEdit
        this.dispose();
}    //GEN-LAST:event_jButton2clickCancelEdit
...
//----------------------SET_TEXT_BOXES_AND_CLASS_ATTRIBUTES---------------------------

public void setEvent(Event value, TableEventModel tableEv) throws PropertyVetoException {

        this.event = value;
        this.tableEv = tableEv;

        Date dateTimeFrom = event.getTimeFrom().getTime();
        Date dateTimeTo = event.getTimeTo().getTime();

        SimpleDateFormat formatter = new SimpleDateFormat("HH:mm");

        jTextNameField.setText(event.getname());
        jTextLabelField.setText(event.getlabel());
        jTextPlaceField.setText(event.getplace());

        try {
                datePicker1.setDate(dateTimeFrom);
                datePicker2.setDate(dateTimeTo);
        } catch (PropertyVetoException ex) {
                logger.error(ex);
        }

        jTextField6.setText(formatter.format(event.getTimeFrom().getTime()));
        jTextField7.setText(formatter.format(event.getTimeTo().getTime()));
